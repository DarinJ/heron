...
public class FilePersistenceStore implements PersistenceStore {
  ...
  public boolean persistJob(BaseJob baseJob) {
    try {
      ...
      Files.write(
          new File(jobPath + "/" + baseJob.name).toPath(),
          BaseJob.getJobDefinitionInJSON(baseJob).getBytes());
    } catch (IOException e) {
  ...
  public boolean removeTask(String taskId) {
    ...
    try {
      Files.deleteIfExists(new File(taskPath + "/"
          + TaskUtils.getJobNameForTaskId(taskId)).toPath());
    } catch (IOException e) {
