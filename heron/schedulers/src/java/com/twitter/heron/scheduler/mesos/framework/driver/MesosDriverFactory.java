...
public class MesosDriverFactory {
  ...
  protected SchedulerDriver mesosDriver;
  ...
  public void start() {
    ...
    if (status != Protos.Status.DRIVER_RUNNING) {
      throw new RuntimeException(String.format("MesosSchedulerDriver start resulted in status: %s. "
  ...
  public void close() {
    if (mesosDriver == null) {
      throw new RuntimeException("Attempted to close a non-initialed driver");
    }
